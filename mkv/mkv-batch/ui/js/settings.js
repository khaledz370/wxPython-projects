...
export class SettingsPage {
  ...
  build() {
    ...
    renderFields(form, [
      ...
      { type: 'check', key: 'backupSameDir', label: 'Backup in the same directory', default: false, show: (v) => v.originalPolicy === 'backup',
        hint: 'Unchecked: use the backup folder at the source drive root.' },
      { type: 'text', key: 'backupDir', label: 'Backup folder', placeholder: 'mkv_old', show: (v) => v.originalPolicy === 'backup',
        hint: 'A folder name such as mkv_old is created at the source drive root. An absolute path such as D:\\mkv_old uses that folder for every file.' },
